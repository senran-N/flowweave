...
result_path="$repo_root/benchmark-results/2026-07-13-b-separated-ingress-observability-v2-smoke.csv"

for required_command in cargo getconf ip nsenter readlink tc unshare; do
    if ! command -v "$required_command" >/dev/null 2>&1; then
        echo "缺少 B ingress v2 实验所需命令：$required_command" >&2
        exit 1
    fi
done

if [[ -e "$result_path" ]]; then
    echo "拒绝覆盖历史 B ingress v2 结果：$result_path" >&2
    exit 1
fi
...
tc qdisc add dev fwbs1 root handle 10: tbf rate 8mbit burst 32kb latency 100ms
tc qdisc add dev fwbs1 parent 10:1 handle 11: netem limit 256 delay 15ms loss 0.1% seed 1101
tc qdisc add dev fwbs2 root handle 20: tbf rate 25mbit burst 32kb latency 100ms
tc qdisc add dev fwbs2 parent 20:1 handle 21: netem limit 256 delay 50ms loss 0.1% seed 2201

export FLOWWEAVE_B_INGRESS_LAB=1
export FLOWWEAVE_B_INGRESS_RECEIVER_PID="$receiver_pid"
export FLOWWEAVE_B_INGRESS_VERSION=v2
