...
第二条命令需要 Linux 的 `unshare`、mount namespace、`ip`、`setpriv`、`jq` 以及当前用户的 `/etc/subuid`、`/etc/subgid` 映射。脚本会先证明已经离开主网络空间，临时创建 `fwvpn0` 并验证 root、未设置 `NoNewPrivileges`、接口 down、MTU 不一致和不存在接口均被拒绝；随后在两个嵌套 network namespace 中创建 veth 和各自的 `fwvpn0`，以无 capability 的设备 owner 运行 server/client，验证 IPv4/IPv6 双向 UDP、分片、两代际复用和退出后重新附着。退出后 mount 与全部网络空间消失。
