...
公网阶段需要一个受控客户端环境和一个受控公网服务端；通常是本地客户端主机加一台远端测试服务器，不要求两台远端服务器。关键条件是客户端同时具备至少两个真实独立出口，并在两端复用相同事件 schema 与健康门控。家庭宽带加手机 4G/5G 是合格示例；同一路由器下的两个地址、Docker 网桥、loopback 和服务端私有 VPC 接口都不算独立公网出口。
