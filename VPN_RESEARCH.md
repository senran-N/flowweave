...
| 身份 | mTLS、证书指纹、双证书轮换、禁用、在线撤销、同步服务端身份重载和同步客户端 TLS 凭据重载已完成；候选先严格验证且配置必须不变，真实门控已在同一 PID/TUN pump 中切换第二张 CA 签发叶证书，并证明旧指纹撤销后旧证书新会话为 `unauthorized` | 完整身份增删/差异预览工具和长期多客户端攻击/并发轮换门控 |
| 会话 | 单活动代际、成功后替换、地址漂移拒绝、启动失败回滚、进程级唯一 TUN reader、离线包丢弃计数、首次 READY 前后 250 ms～30 秒随机指数退避重连、非特权 netlink 提前唤醒、同一已连接 QUIC 内的显式路径原位替换，以及同进程客户端凭据代际切换已完成；真实门控覆盖客户端先启动/服务端后出现、身份与凭据恢复、单链 down/up、至少 8 秒退避的事件唤醒、服务端重启，并用独立 NAT router 证明 SNAT mapping rebinding 时 stable ID 与 `FWC1` generation 不变 | 长期静默与多客户端并发切换矩阵 |
...
| 验证 | fixed-tcp soak、真实 mTLS/FWC1/DATAGRAM loopback，隔离真实 TUN 的 UDP/TCP/ICMP/MTU/`SIGKILL`，产品进程 READY/正常停止/离线停止，UID 外层逃生，IPv4 NAT/IPv6 无 NAT，双端同步 reload、同 PID 客户端真实双证书切换/旧证书拒绝/离线凭据恢复、原 PID 身份恢复、完整重连、netlink 提前唤醒，以及双 outer-path 的 PathId 增删、显式源 IP 保持和固定 SNAT 40000→40001 原位恢复，另有 route/rule/nft/sysctl 冲突、漂移、崩溃恢复和真实 user-systemd 生命周期 | 独立运营商出口、多客户端、多日和跨版本门控 |
...
- 第十小步当时仍不实现后台重连：client 外层失联会按 10 秒 connection idle 上限结束并以非零状态退出；后续 M2.3 第一小步现已用进程级 packet pump 与 supervisor 取代这一产品语义。server 仍保持顺序接入循环直到本地停止。
- 真实门控复用第九小步 helper 准备的双 namespace TUN：启动产品 server/client、分别观察 READY，完成双栈大 ICMP 或 UDP 流量，SIGTERM client 后确认 server 保持运行，再 SIGTERM server；双方必须 `stopped`、退出码为零、TUN 可由 helper cleanup。第十小步当时另以切断外层 veth 验证 client 非零退出且不得发送正常 stopped；该故障语义后来由 M2.3 第一小步的内部重连取代。
...
- SIGTERM/Ctrl-C 在 DNS/QUIC 启动阶段会取消尚未 READY 的连接尝试；运行期则先通知 STOPPING，再请求桥接、连接和 Endpoint 有界停止。clean stop 只有资源收敛后才输出 `stopped`。第十小步时 SIGHUP 同样先有界收敛并返回稳定 `vpn_process_reload_unsupported`；后续 M2.2 第一小步先为服务端接入身份 reload，M2.3 第五小步又为客户端接入严格 TLS 凭据 reload。第十小步时 Endpoint 或连接自行结束均为非零错误；M2.3 第一小步后来允许 READY 后可恢复故障留在进程内，第二小步又允许 READY 前的网络/远端可恢复错误重试，只有配置、协议、本地资源不变量和 worker 等不可恢复错误仍非零退出。
- `scripts/run_vpn_tun_lab.sh` 已在一次性 user+mount+network namespace 中真实运行产品 server/client：双方均为 UID/GID 1000、空 capability、`NoNewPrivileges`。client 在外层不可达、尚未 READY 的连接阶段收到 SIGTERM 后 5 秒内零退出，只输出 `stopped` 而不误报 `ready`；正常 READY 后 IPv4/IPv6 各三次 1300 字节 ICMP 全部往返。SIGTERM client 后只输出一次 `stopped`、零退出且 server 保持运行；server 随后同样正常停止。第十小步当时重启双方并切断 client 外层 veth 后，client 按既定 10 秒 connection idle 边界非零退出、没有 `stopped`；M2.3 第一小步现已在同一纵切中证明原客户端 PID 和 route sidecar 保持、链路恢复后自动重连。最终 helper cleanup 证明两端 TUN 消失，主网络空间无网络副作用。
...
- `scripts/run_vpn_tun_lab.sh` 在真实产品 client 输出 READY 后才调用 activate：route lookup 证明 UID 0 到外层服务端地址命中 `fwvpn0`，产品 UID 1000 仍命中原 `fwclient0`；随后双栈 1300 字节 ICMP 继续通过，证明 QUIC 没有自锁。第十一小步当时让外层断链以非零退出并撤销 route；M2.3 第一小步现已把 READY 后的可恢复断链改为保持 route、离线丢包并在原进程重连，只有正常/不可恢复退出才进入 deactivate。主网络空间无副作用。
...
至此，M2.1 的数据面、最小特权网络事务和安装生命周期骨架已经闭合；后续 M2.3 前两小步又补上首次 READY 前后的选择性内部重试和 systemd 启动截止收敛，但整体仍不等于生产可部署 VPN：DNS 接管与回退、网络变化事件唤醒、真实宿主特权安装/卸载、发行版兼容、多日 soak 和跨版本升级回退证据仍缺失。
...
当前地基：mTLS、指纹注册、双指纹重叠、禁用、静态地址、目标 CIDR、严格身份文件、单活动代际、成功后替换、在线撤销、跨代际共享速率桶、服务端按身份数据句柄、客户端 `ACCEPT` 工厂、外层 fragment 准入、真实重组、协商包长、原子全局账本、源地址防伪、双向 ACL、真实 NoQ DATAGRAM 运行器，以及服务端身份/客户端 TLS 凭据两套同步 reload 已接入真实 TUN 产品进程。完整身份增删工具和长期多客户端攻击/并发轮换矩阵仍未完成。详见 `VPN_IDENTITY.md`。

M2.2 第一小步固定为“同步、网络兼容的服务端身份重载”：

- 正式 `systemctl reload flowweave-vpn-server` 不使用异步 `kill -HUP`。服务端在 systemd 创建、仅 `flowweave` 可访问的 `0700` RuntimeDirectory 内监听 `0600` Unix 控制 socket；同一非特权二进制的 `reload-server <socket>` 子命令发送固定有界请求、等待服务端完成真实重载并按结果退出，使 systemd 能同步报告成功或失败。
- 服务端仍支持管理员直接发送 SIGHUP，但它只触发同一内部重载函数；同步、可脚本化的运维合同以控制 socket 为准。本步当时把客户端证书/CA/私钥在线重载后置到专门的 TLS 身份切换阶段；该阶段现已由 M2.3 第五小步完成，仍不允许把本地提交假装成远端已接受的新会话。
- 候选身份文件必须重新通过普通文件、权限、大小、严格 JSON、全局冲突和逐身份 limits 检查；同时复用初次 bootstrap 的产品约束，拒绝任一身份 `max_reassembly_bytes` 超过服务端 `global_reassembly_bytes`。任何失败都必须在取得协调器写锁和关闭会话前返回，旧注册表、quota、活动代际和网络对象完全不变。
- 在线重载只能提交不改变现有 root 网络事务的候选：服务端虚拟地址、全部客户端虚拟地址集合必须与当前注册表完全相同；启用 forwarding 时，enabled 身份的 IPv4/IPv6 地址集合也必须完全相同。地址增加/删除/迁移，或会改变 nft 精确源地址集合的启用/禁用，返回稳定“需要受控重启”错误，不能让数据面、TUN host route 与 nft sidecar 出现三份不同真值。
- 在上述网络合同不变时，允许原子修改证书指纹重叠槽、`client_id` 归属、目标 ACL 和资源 limits。提交时继续使用 `VpnSessionCoordinator::replace_registry`：仍被授权且策略相同的健康会话保留；活动指纹撤销或重新归属立即关闭为 `identity_revoked`；ACL/limits/地址语义变化立即关闭为 `policy_changed`；失败候选不影响旧会话。
- 控制协议不传地址、指纹、私钥或完整错误，只接受精确 `FWR1` 请求和单字节成功/拒绝响应；socket 路径、请求长度、响应长度和 I/O 时间都有上限。服务端报告只增加脱敏的 reload 成功/失败计数和稳定 systemd `STATUS`，stdout 的唯一 `ready` / `stopped` 行合同不变。明确拒绝保证旧内存状态未变；connect/I/O/timeout/非法响应属于提交结果不确定，运维方必须查 journal 并幂等重试同一磁盘候选。
- 门控必须覆盖无效 JSON、超全局预算、网络地址漂移、forwarding enabled 集合漂移均同步失败且旧真实 mTLS/TUN 会话继续传输；双指纹重叠成功且保留会话；删除当前指纹成功并关闭旧连接；恢复原身份后客户端可建立新连接。M2.2 当时由进程退出观察关闭结果，M2.3 第一小步现已改为原进程进入离线重连。所有真实网络变化继续只发生在一次性 namespace。

M2.2 第一小步实现证据：

- `VpnServerProductBootstrap::reload_identities` 重新加载原始 `identity_file`，复用初次 bootstrap 的全局重组预算检查，并分别比较服务端/全部客户端地址拓扑与 forwarding enabled 地址集合；候选只有在两类 root 网络计划都保持兼容后才调用协调器原子替换。专用稳定错误区分资源超限、TUN 拓扑变化和 forwarding 变化，均不输出地址或指纹。
- `flowweave-vpn server` 可在 `FLOWWEAVE_VPN_SERVER_RELOAD_SOCKET` 指定的绝对 socket 上提供固定 `FWR1` 请求；父目录必须由当前有效 UID 独占且不可由 group/other 访问，socket 固定为 `0600`，Drop 只按 device/inode 删除自己创建的 socket。请求、单字节响应和每段 I/O 均有 2 秒上限。
- `flowweave-vpn reload-server <socket>` 使用同一固定协议同步等待提交结果；服务端正式 unit 通过 `RuntimeDirectory=flowweave-vpn-server`、`RuntimeDirectoryMode=0700` 和非特权 `ExecReload=` 接线。失败候选让 reload 命令非零退出但不终止主进程；直接 SIGHUP 也调用同一内部函数。客户端在本步仍明确不支持，M2.3 第五小步现已用独立 `reload-client` 控制合同补齐。
- 服务端进程报告新增 reload 成功/失败计数，journal 输出脱敏稳定行并更新 systemd `STATUS`；stdout 仍只有唯一 `ready` / `stopped`。单元测试证明 `0600` socket、共享目录拒绝、成功响应、坏 JSON 拒绝和旧注册表保留。
- 真实 user-systemd 门控的 `ExecReload` 已改为调用实际 `flowweave-vpn reload-server` 二进制：成功 reload 同步完成；拒绝响应使 `systemctl reload` 返回非零，数据主进程继续 active，随后正常停止仍执行 `deactivate → cleanup`；reload 命令继承空 capability 与 `NoNewPrivileges`，不使用 `+` 获得 root。
- `scripts/run_vpn_tun_lab.sh` 在 forwarding/NAT 已激活的三 namespace 产品纵切中通过：坏 JSON、客户端地址漂移和 enabled 集合漂移均同步拒绝，旧 mTLS/TUN 会话继续双栈传输；增加第二指纹后旧会话保持；只保留替代指纹后当前连接关闭且服务端继续运行；恢复原身份并 reload 后客户端重新完成 mTLS、FWC1 和双栈流量。M2.2 当时客户端进程随连接退出，M2.3 第一小步现已证明同一客户端 PID 留在 supervisor 中恢复。主网络空间没有副作用。
...
M2.3 第一小步固定为“首次 READY 后的安全内部重连”，不把 systemd 外部重启伪装成连接恢复：

- 客户端进程启动一个跨连接代际存活的 TUN packet pump，始终是唯一 reader。当前代际通过带单调 generation 的 attachment 安装 `VpnPacketSender`；代际结束先按 generation 摘除 sender，再关闭 DATAGRAM runtime。离线期间 pump 继续读取但不缓存、不重放，按包数和字节累计稳定指标。
- 首次连接仍必须完整通过 DNS、TLS 1.3 标准名称验证、MPQUIC、全部 primary/additional 路径、`FWC1 ACCEPT` 地址精确比对和 DATAGRAM 后才 READY。READY 之后的可恢复连接关闭不退出进程、不撤销 policy route；每次尝试重新调用同一 Endpoint 构造，因此重新解析服务端地址并重新执行证书、能力、路径和控制握手验证，不允许单路径静默降级。
- 重连基准延迟从 250 ms 指数增长到 30 秒，使用系统随机源加入抖动；服务端 `REJECT.retry_after_secs` 若更长则作为下限。每次成功连接后新一轮故障重新从初始退避开始。所有等待、DNS、连接、握手和路径操作继续可被 SIGTERM/Ctrl-C 取消。
- 可恢复分类限定为连接关闭/失败、活动代际 stale、DATAGRAM 发送失败和 packet-id 耗尽；TUN reader/write、pump、资源不变量或 worker 故障仍让进程失败退出，由 systemd 反向撤销网络事务。本步当时仍拒绝客户端 SIGHUP；M2.3 第五小步现已让 SIGHUP 与同步控制复用严格候选函数，并以新 mTLS/FWC1 activation 作为真正完成证据。
- 进程报告新增累计重连尝试/成功/失败、离线毫秒和离线丢包；journal 使用不含地址、证书或 `client_id` 的稳定 `vpn_client_connection_lost`、`vpn_client_reconnect_waiting`、`vpn_client_reconnect_failed`、`vpn_client_reconnected` 行，systemd `STATUS` 区分 reconnecting/reconnected。stdout 仍只有首次 `ready` 和最终正常 `stopped`。

第一小步实现证据：

- 单元测试证明无 active sender 时每个 TUN 包都被立即读取并计入 offline drop，且 250 ms 起步、30 秒上限、随机抖动区间和服务端更长 retry-after 下限保持稳定；根项目非忽略门禁增至 `219/219`。
- `scripts/run_vpn_tun_lab.sh` 在 route 与 forwarding/NAT 保持 active 时撤销当前客户端指纹：旧代际关闭，原客户端 PID 留在 supervisor 中并丢弃离线 ICMP；恢复身份并 reload 后同一 PID 重新完成 mTLS/FWC1，双栈流量恢复且日志证明 offline drop 非零。
- 同一三 namespace 纵切把 client 外层 veth 置 down，等待真实 10 秒 connection idle 收敛后确认进程和 route sidecar 仍存活；接口恢复后原 PID 自动重连并恢复双栈流量。随后正常停止服务端、启动新服务端进程，客户端再次自动恢复；第二次停止服务端后，客户端在离线退避中收到 SIGTERM 仍零退出并输出唯一 `stopped`，最终 `deactivate → cleanup` 无遗留。

M2.3 第二小步固定为“首次 READY 前的选择性内部重试与 systemd 启动截止协同”：

- 首次连接立即尝试一次；DNS 暂时失败或没有兼容地址、本地已配置源地址尚未出现、QUIC 连接/握手暂时失败、可恢复路径验证失败、控制流 I/O 失败，以及 `Unauthorized`、`IdentityDisabled`、`AddressInUse`、`ResourceLimit`、`PolicyRejected` 服务端拒绝，才进入与 READY 后相同的 250 ms～30 秒随机指数退避，并尊重更长 `retry_after_secs`。每次都重建 Endpoint、重新解析 DNS、执行标准名称校验、验证全部显式路径和 `FWC1`，不能在首次成功前静默降级。
- 无效服务端名称、QUIC 版本不兼容、TLS 证书/名称校验失败、ALPN/MPQUIC/DATAGRAM 不兼容、无效控制消息、服务端地址分配漂移、重复 packet-pump attachment、本地权限/资源不变量和 worker 故障仍快速失败。底层连接错误只映射为版本、传输暂不可用、传输协议、密码学验证、对端关闭、重置、超时、本地关闭或 CID 耗尽等脱敏类别，不把证书内容、名称或地址带入日志，也不会把确定性配置错误包装成“网络稍后会恢复”。
- 首次 READY 前退避期间 packet pump 仍是唯一 TUN reader，包立即丢弃且不在成功后重放；SIGTERM/Ctrl-C 可取消等待和正在进行的 DNS/QUIC 尝试，正常停止仍只输出 `stopped` 而不误报 `ready`。进程报告增加首次连接尝试/失败/耗时；journal 增加脱敏稳定 `vpn_client_startup_failed`、`vpn_client_startup_waiting`，systemd `STATUS` 标记等待首次连接。
- 程序本身不猜测安装器的启动策略：正式 client unit 继续用 `TimeoutStartSec=90s` 限制单次 activation。若始终未 READY，systemd 终止主进程；主进程有界收敛 packet pump 后，`ExecStopPost` 仍按 `deactivate → cleanup` 执行，且 `ExecStartPost=activate-client` 从未运行。`Restart=on-failure` 再决定是否开始下一轮，不用无限扩展启动截止。

第二小步实现证据：

- 可恢复分类单元测试锁定网络/远端错误与配置/协议/本地不变量的分界，并验证服务端拒绝的 `retry_after_secs`；根项目非忽略门禁现为 `225/225`。
...
M2.3 第五小步固定为“客户端 TLS 凭据同步 reload 与同进程代际切换”，不得用 systemd 重启伪装在线轮换，也不得在本地校验失败时中断旧健康会话：

- 客户端候选重新严格加载同一路径的完整 product config，并要求它与当前内存配置完全相等；服务端名称、TUN、虚拟地址、ACL、limits、outer paths 或凭据路径任一变化都返回“需要受控重启”。只有现有服务端 CA、客户端证书和 PKCS#8 私钥文件的内容可变化；普通文件、1 MiB 上限、私钥权限、CA 解析和证书/私钥匹配继续全部前置。候选失败不替换 bootstrap、不关闭当前连接；三份内容的长度分隔 SHA-256 只在内存判断幂等，不进入日志。
- 新 bootstrap 必须克隆旧 `VpnClientDataPathFactory` 和同一 runtime lease，使本地速率桶、全局重组预算、指标以及“至多一个活动桥接”跨证书切换保持；内容未变的 reload 幂等成功且不重连。有效变化先提交内存，再有界关闭旧 Endpoint 代际并立即使用新 TLS 配置执行 DNS、标准名称校验、MPQUIC 全路径和 `FWC1`。PID、TUN packet pump、policy route 与 READY 状态保持，stdout 不重复输出 `ready`；QUIC stable ID 和 FWC1 generation 必须形成新代际，不能沿用旧 TLS 会话。
- 客户端在 systemd 创建的独立 `0700` RuntimeDirectory 内绑定 `0600` socket，固定 `FWR2` 请求和单字节响应；`flowweave-vpn reload-client <socket>` 与正式非特权 `ExecReload` 同步区分本地提交/拒绝。直接 SIGHUP 复用同一候选函数，但没有同步调用方，因此正式运维合同以 socket 为准。成功响应只证明内存提交；远端真正接受必须由 `vpn_client_credentials_active`、新 connection/session 观测和真实流量证明。
- reload 请求在首次连接、在线运行、离线退避和正在重连四种阶段都可受理。已提交候选若尚未被服务端登记，不把密码学/授权失败当成进程永久错误：客户端保持唯一 TUN reader、离线丢包和有界退避，reload socket 继续可修正候选。进程报告增加 `credential_generation`、reload/失败/activation 计数；稳定日志只含 generation、connection stable ID、session generation、布尔变化和错误类别，不含证书、私钥、路径、地址、指纹或 `client_id`。

第五小步实现证据：

- 两项确定性单元测试证明：不变候选幂等；配置漂移拒绝；证书/私钥错配拒绝且旧 revision 保留；有效候选才递增 generation；候选与当前 bootstrap 共享 runtime lease 和客户端数据路径状态。正式 client unit 的 RuntimeDirectory、环境变量和非特权 `reload-client` ExecReload 由静态合同锁定。根项目当前非忽略全目标门禁为 `232/232`。
- `scripts/run_vpn_tun_lab.sh` 生成同一客户端 CA 签发、密钥不同的第二张真实叶证书。先只安装新证书配旧私钥，reload 必须明确拒绝、旧连接和双栈流量不受影响；服务端登记真实新指纹后，客户端同步 reload 在同一 PID/TUN pump 中建立新 QUIC stable ID 与更高 FWC1 generation，`connection_lost` 计数不变且 stdout 仍只有一个 `ready`。
- 服务端随后只保留新指纹，当前新证书会话继续；客户端再 reload 已撤销的旧证书时，新 FWC1 会话明确收到 `unauthorized`，进程保持离线和控制 socket 可用。离线 reload 新证书后 credential generation 从 2 前进到 3、同 PID 恢复双栈流量；最后主动撤销/恢复当前新指纹，既有身份撤销、离线丢包和重连门控继续通过。所有文件、socket 和网络对象仍只存在于一次性 namespace。

仍未完成：长期静默、多客户端并发轮换与独立运营商故障矩阵。前五小步关闭了“首次和后续连接能否安全重建、旧包是否重放、等待能否由真实网络恢复安全缩短、已连接会话能否保持显式源完成真实 NAT rebinding、客户端证书能否不重启进程完成严格切换、systemd 启动怎样收敛”的基础合同，仍不等于 M2.3 完成。
