...
真实 NoQ DATAGRAM 运行器也已接线：上行/下行完整 IP 包通过包数和字节双重有界队列与未来 TUN 侧隔离；出站整包的所有 fragment 以一个原子 token-bucket 批次准入，避免只发半包前就部分扣额；实际发送使用等待型 DATAGRAM 背压，入站队列满则丢新包并继续读取 QUIC。运行器包含 250 ms 默认重组过期 tick、取消安全、连接/代际终止原因、packet-id 耗尽保护和脱敏原子指标；同一数据句柄只能绑定一次。真实 loopback NoQ 测试已完成双向 IPv4/IPv6 分片传输、队列满不阻塞、无流量 stale 退出和运行器关闭清理。它仍未接入产品命令或 TUN，因此不是可部署 VPN。
...
当前地基：真实 NoQ DATAGRAM 双向运行器、有界 TUN 边界队列、分片批次准入、周期过期和稳定终止报告已完成 loopback 验证。仍需实现客户端策略/身份构造、产品 `vpn-server` / `vpn-client` 配置与命令、root oneshot TUN 准备单元、非特权 TUN 读写任务，以及 namespace 端到端矩阵。
...
当前地基：mTLS、指纹注册、双指纹重叠、禁用、静态地址、目标 CIDR、严格身份文件、单活动代际、成功后替换、在线撤销、跨代际共享速率桶、按身份分片的数据句柄、外层 fragment 准入、真实重组、原子全局账本、源地址防伪、双向 ACL 和真实 NoQ DATAGRAM 运行器已实现纯内存/loopback 合同。TUN 产品接线、运维工具、SIGHUP 接线和长期多客户端攻击矩阵仍未完成。详见 `VPN_IDENTITY.md`。
