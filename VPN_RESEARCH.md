...
| 验证 | fixed-tcp soak、真实 mTLS/FWC1/DATAGRAM loopback，隔离真实 TUN 的 UDP/TCP/ICMP/MTU/失联/`SIGKILL`，产品进程 READY/正常停止/断链非零退出，UID 外层逃生，IPv4 NAT/IPv6 无 NAT，身份 reload 成功/拒绝/撤销/恢复，route/rule/nft/sysctl 冲突、漂移和崩溃恢复，以及真实 user-systemd 的五类启动/退出顺序与同步 reload | 多路径切换、独立出口、多日和跨版本门控 |
...
- SIGTERM/Ctrl-C 在 DNS/QUIC 启动阶段会取消尚未 READY 的连接尝试；运行期则先通知 STOPPING，再请求桥接、连接和 Endpoint 有界停止。clean stop 只有资源收敛后才输出 `stopped`。第十小步时 SIGHUP 同样先有界收敛并返回稳定 `vpn_process_reload_unsupported`；后续 M2.2 第一小步已为服务端接入身份 reload，客户端仍保持这一明确拒绝语义。Endpoint 或连接自行结束均为非零错误。
...
- 服务端仍支持管理员直接发送 SIGHUP，但它只触发同一内部重载函数；同步、可脚本化的运维合同以控制 socket 为准。客户端证书/CA/私钥重载仍后置到重连 supervisor，不允许用 SIGHUP 假装已切换正在使用的 TLS 身份。
...
- 门控必须覆盖无效 JSON、超全局预算、网络地址漂移、forwarding enabled 集合漂移均同步失败且旧真实 mTLS/TUN 会话继续传输；双指纹重叠成功且保留会话；删除当前指纹成功并使旧连接非零退出；恢复原身份后新连接可重新建立。所有真实网络变化继续只发生在一次性 namespace。
...
- `scripts/run_vpn_tun_lab.sh` 在 forwarding/NAT 已激活的三 namespace 产品纵切中通过：坏 JSON、客户端地址漂移和 enabled 集合漂移均同步拒绝，旧 mTLS/TUN 会话继续双栈传输；增加第二指纹后旧会话保持；只保留替代指纹后当前客户端非零退出且服务端继续运行；恢复原身份并 reload 后客户端重新完成 mTLS、READY 和双栈流量。主网络空间没有副作用。
