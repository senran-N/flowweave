...
当前地基：真实 NoQ DATAGRAM 双向运行器、有界 TUN 边界队列、分片批次准入、周期过期、稳定终止报告和客户端 `ACCEPT` 数据句柄工厂已完成组合 loopback 验证。仍需实现产品 `vpn-server` / `vpn-client` 配置与命令、root oneshot TUN 准备单元、非特权 TUN 读写任务，以及 namespace 端到端矩阵。
