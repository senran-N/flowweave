...
| 运维 | fixed-tcp 首次安装/systemd/JSONL/健康门控；VPN 已有非特权产品命令、严格 READY/systemd notify 和具备版本状态、锁、幂等、回滚的最小网络 helper | 安装级 VPN systemd、TUN/策略路由/NAT 原子编排、升级迁移、回退、配置检查和告警出口 |
| 验证 | fixed-tcp soak、真实 mTLS/FWC1/DATAGRAM loopback，隔离真实 TUN 的 UDP/TCP/ICMP/MTU/失联/`SIGKILL`，产品进程 READY/正常停止/断链非零退出，以及 root helper 的冲突/漂移/崩溃恢复 | 默认路由/NAT、多路径切换、独立出口、多日和跨版本门控 |
...
当前地基：真实 NoQ DATAGRAM 双向运行器、有界 TUN 边界队列、分片批次准入、周期过期、稳定终止报告和客户端 `ACCEPT` 数据句柄工厂已完成组合 loopback 验证。上述“预附着包设备桥接”第一小步也已实现：只对传入 fd 设置 nonblocking，Unix packet socket 实测保持双向包边界；超限包被丢弃后后续合法包继续传输，运行器队列满只丢当前设备包并精确计数，代际 stale 原因能穿透到桥接报告，设备写失败会停止本代际且不产生续包。后续第六至第十小步又完成真实 namespace 端到端矩阵、最小 root 网络事务和非特权 `flowweave-vpn server|client` 生命周期；仍需实现安装级 systemd 编排、默认/选择性路由及服务端 forwarding/NAT。
