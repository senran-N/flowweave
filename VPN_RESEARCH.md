...
| 能力 | 当前证据 | 生产 VPN 所需状态 |
| --- | --- | --- |
| 数据面 | `FWI1`、真实重组、有界队列、双方 NoQ DATAGRAM、包设备桥接和真实 TUN 已在隔离双 namespace 中串通 UDP、TCP、双栈 ICMP、精确 MTU、连续代际、外层失联与 READY 后 policy routing | 服务端 UDP/IP forwarding/NAT、更完整故障矩阵和无全局队头阻塞 |
...
| 地址与策略 | 静态虚拟 IPv4/IPv6、源地址防伪、双向 ACL、每身份配额、客户端 `ACCEPT` 工厂，以及由同一 `allowed_destinations` 派生的独立 TUN 路由表与原子撤销已完成隔离验证 | 服务端宿主机转发、管理流量豁免策略和 DNS 接管 |
| 权限 | 非 root、无可重新启用的 `CAP_NET_ADMIN`、`NoNewPrivileges` 数据进程只附着既有 TUN 的边界已实测；独立 root helper 已事务化准备/清理 TUN 并激活/撤销 client policy routes | 安装级 systemd oneshot/notify 编排与 forwarding/NAT |
| 运维 | fixed-tcp 首次安装/systemd/JSONL/健康门控；VPN 已有非特权产品命令、严格 READY/systemd notify，以及具备版本状态、锁、幂等、回滚的 TUN 与 client route helper | 安装级 VPN systemd、NAT/forwarding 原子编排、升级迁移、回退、配置检查和告警出口 |
| 验证 | fixed-tcp soak、真实 mTLS/FWC1/DATAGRAM loopback，隔离真实 TUN 的 UDP/TCP/ICMP/MTU/失联/`SIGKILL`，产品进程 READY/正常停止/断链非零退出，UID 外层逃生与 route/rule 冲突/漂移/崩溃恢复 | 服务端 NAT、多路径切换、独立出口、多日和跨版本门控 |
...
当前地基：真实 NoQ DATAGRAM 双向运行器、有界 TUN 边界队列、分片批次准入、周期过期、稳定终止报告和客户端 `ACCEPT` 数据句柄工厂已完成组合 loopback 验证。上述“预附着包设备桥接”第一小步也已实现：只对传入 fd 设置 nonblocking，Unix packet socket 实测保持双向包边界；超限包被丢弃后后续合法包继续传输，运行器队列满只丢当前设备包并精确计数，代际 stale 原因能穿透到桥接报告，设备写失败会停止本代际且不产生续包。后续第六至第十一小步又完成真实 namespace 端到端矩阵、TUN/root route 网络事务、非特权 `flowweave-vpn server|client` 生命周期和 READY 后 client policy routing；仍需实现安装级 systemd 编排及服务端 forwarding/NAT。
...
第六小步完成的真实 IPv4/IPv6 UDP 与分片纵切，已经由下面第七、八小步继续补齐常见协议、MTU、外层中断和异常退出清理；随后第九小步把内层地址/host route 收敛为 root 网络事务，第十、十一小步又补齐产品 READY 与客户端默认/选择性路由事务。服务端 forwarding/NAT 和 DNS 仍必须按后续独立合同实现。
...
- server READY 只能在真实 UDP Endpoint 绑定成功且 accept worker 已启动后发出；client READY 必须晚于 DNS、标准 TLS 名称校验、MPQUIC、全部显式路径验证、`FWC1 ACCEPT` 地址精确比对、DATAGRAM 和包桥接启动。该顺序是未来 systemd `ExecStartPost` 或独立 route activator 的唯一许可点。
...
- `activate-client` 只能由未来 Type=notify systemd 单元的 `ExecStartPost` 或等价受控 route activator 在 client READY 后调用；`deactivate-client` 由 `ExecStopPost` 在正常停止、启动失败和异常退出后调用。helper 本身仍不信任 stdout 文本，也不启动数据进程；实验脚本显式模拟该时序。
...
第十一小步完成的是可回滚的客户端流量接管原语，不是安装级自动编排，也没有可转发的公网服务端。下一步实现服务端 forwarding/NAT 的同类 journal、现有 sysctl/nftables 对象归属保护和双栈/无 NAT 边界；待两端事务都通过后，再用 Type=notify systemd 把 prepare → data READY → activate 与 stop/failure → deactivate → cleanup 串成可安装、可回退单元。DNS 继续后置。
