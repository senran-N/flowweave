...
| 验证 | fixed-tcp soak、真实 mTLS/FWC1/DATAGRAM loopback，以及隔离真实 TUN 的 UDP/TCP/ICMP/MTU、外层断网和 `SIGKILL` 后重附着 | root oneshot、默认路由/NAT、多路径切换、独立出口、多日和跨版本门控 |
...
第六小步完成的真实 IPv4/IPv6 UDP 与分片纵切，已经由下面第七、八小步继续补齐常见协议、MTU、外层中断和异常退出清理。M2.1 剩余关键工作是把当前脚本中的管理命令收敛为可审计、可撤销的 root oneshot 状态机；服务端 forwarding/NAT、客户端默认路由和 DNS 必须在该状态机及其反例门控之后单独实现。
...
第八小步关闭了 M2.1 的第一组失联与进程异常退出门控，但还不等于产品重连或可部署 VPN。下一步先实现 root oneshot 的版本化状态文件、幂等 prepare/cleanup 和失败回滚，再在隔离 namespace 中接入服务端 forwarding/NAT 与客户端选择性路由；内部 supervisor、多路径恢复和长期 soak 仍归 M2.3。
