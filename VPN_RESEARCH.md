...
- SIGTERM/Ctrl-C 在 DNS/QUIC 启动阶段会取消尚未 READY 的连接尝试；运行期则先通知 STOPPING，再请求桥接、连接和 Endpoint 有界停止。clean stop 只有资源收敛后才输出 `stopped`。第十小步时 SIGHUP 同样先有界收敛并返回稳定 `vpn_process_reload_unsupported`；后续 M2.2 第一小步已为服务端接入身份 reload，客户端仍保持这一明确拒绝语义。第十小步时 Endpoint 或连接自行结束均为非零错误；M2.3 第一小步后来允许 READY 后可恢复故障留在进程内，第二小步又允许 READY 前的网络/远端可恢复错误重试，只有配置、协议、本地资源不变量和 worker 等不可恢复错误仍非零退出。
...
当前地基：mTLS、指纹注册、双指纹重叠、禁用、静态地址、目标 CIDR、严格身份文件、单活动代际、成功后替换、在线撤销、跨代际共享速率桶、服务端按身份数据句柄、客户端 `ACCEPT` 工厂、外层 fragment 准入、真实重组、协商包长、原子全局账本、源地址防伪、双向 ACL、真实 NoQ DATAGRAM 运行器和同步服务端身份 reload 已接入真实 TUN 产品进程。完整身份增删工具、客户端在线 TLS 身份切换和长期多客户端攻击矩阵仍未完成。详见 `VPN_IDENTITY.md`。
...
- 服务端仍支持管理员直接发送 SIGHUP，但它只触发同一内部重载函数；同步、可脚本化的运维合同以控制 socket 为准。客户端证书/CA/私钥在线重载仍后置到专门的 TLS 身份切换阶段，不允许用 SIGHUP 假装已切换正在使用或下一次重连将读取的凭据。
...
- `flowweave-vpn reload-server <socket>` 使用同一固定协议同步等待提交结果；服务端正式 unit 通过 `RuntimeDirectory=flowweave-vpn-server`、`RuntimeDirectoryMode=0700` 和非特权 `ExecReload=` 接线。失败候选让 reload 命令非零退出但不终止主进程；直接 SIGHUP 也调用同一内部函数，客户端 SIGHUP 仍明确不支持。
...
- 可恢复分类限定为连接关闭/失败、活动代际 stale、DATAGRAM 发送失败和 packet-id 耗尽；TUN reader/write、pump、资源不变量或 worker 故障仍让进程失败退出，由 systemd 反向撤销网络事务。客户端 SIGHUP 仍明确不支持，不能假装已切换 TLS 凭据。
