...
| 会话 | 单活动代际、成功后替换、地址漂移拒绝、启动失败回滚、旧运行器退出和唯一包设备读者租约已完成 | Endpoint 生命周期、有界指数退避、网络恢复、路径重建和离线包丢弃计数 |
