...
| 能力 | 当前证据 | 生产 VPN 所需状态 |
| --- | --- | --- |
| 数据面 | `FWI1`、真实重组、有界队列和双方 NoQ DATAGRAM 已在 loopback 串通，尚无 TUN | TUN 三层 IP、UDP/IP 宿主机转发和无全局队头阻塞 |
...
| 验证 | fixed-tcp soak 加真实 mTLS/FWC1/DATAGRAM IPv4/IPv6 loopback 组合测试 | TUN 端到端、UDP/IPv6/MTU、故障注入、独立出口、多日和跨版本门控 |
...
当前地基：真实 NoQ DATAGRAM 双向运行器、有界 TUN 边界队列、分片批次准入、周期过期、稳定终止报告和客户端 `ACCEPT` 数据句柄工厂已完成组合 loopback 验证。上述“预附着包设备桥接”第一小步也已实现：只对传入 fd 设置 nonblocking，Unix packet socket 实测保持双向包边界；超限包被丢弃后后续合法包继续传输，运行器队列满只丢当前设备包并精确计数，代际 stale 原因能穿透到桥接报告，设备写失败会停止本代际且不产生续包。仍需实现产品 `vpn-server` / `vpn-client` 配置与命令、受控 TUN fd 获取/`IFF_NO_PI` 验证、root oneshot TUN 准备单元，以及真实 namespace 端到端矩阵。
