...
#[test]
fn path_scheduling_default_remains_lowest_path_first() -> TestResult {
    let _guard = subscribe();
...
#[test]
fn ack_ecf_probes_unmeasured_paths_with_normal_application_data() -> TestResult {
    let _guard = subscribe();
    let mut pair = pair_with_scheduling_policy(MultipathSchedulingPolicy::AckEcf);
    let path_1 = open_available_second_path(&mut pair)?;
    queue_scheduler_test_data(&mut pair);
...
#[test]
fn ack_ecf_prefers_the_lowest_predicted_completion_time() -> TestResult {
    let _guard = subscribe();
    let mut pair = pair_with_scheduling_policy(MultipathSchedulingPolicy::AckEcf);
    let path_1 = open_available_second_path(&mut pair)?;
    let now = pair.time;
    pair.conn_mut(Client).set_path_ack_ecf_metrics_for_test(
        PathId::ZERO,
        now,
    ...
    assert!(
        !pair
            .conn(Client)
            .path_ack_ecf_metrics_for_test(PathId::ZERO, now)?
            .2,
        "control-only prepass must not mark an unselected data path app-limited"
    );
    Ok(())
}

#[test]
fn ack_ecf_waits_when_a_blocked_preferred_path_is_meaningfully_earlier() -> TestResult {
    let _guard = subscribe();
    let mut pair = pair_with_scheduling_policy(MultipathSchedulingPolicy::AckEcf);
    let path_1 = open_available_second_path(&mut pair)?;
    let now = pair.time;
    ...
    let before = pair.path_stats(Client, path_1).unwrap().udp_tx.datagrams;
    let mut buf = Vec::new();
    assert!(
        pair.poll_transmit(Client, NonZeroUsize::MIN, &mut buf)
            .is_none()
    );
    assert_eq!(
        pair.path_stats(Client, path_1).unwrap().udp_tx.datagrams,
        before
    );
    Ok(())
}
...
#[test]
fn ack_ecf_uses_an_alternate_within_prediction_uncertainty() -> TestResult {
    let _guard = subscribe();
    let mut pair = pair_with_scheduling_policy(MultipathSchedulingPolicy::AckEcf);
    let path_1 = open_available_second_path(&mut pair)?;
    let now = pair.time;
...
#[test]
fn ack_ecf_keeps_available_paths_ahead_of_backups() -> TestResult {
    let _guard = subscribe();
    let mut pair = pair_with_scheduling_policy(MultipathSchedulingPolicy::AckEcf);
    let path_1 = open_available_second_path(&mut pair)?;
...
#[test]
fn ack_ecf_single_path_uses_noq_default_path() {
    let _guard = subscribe();
    let mut pair = pair_with_scheduling_policy(MultipathSchedulingPolicy::AckEcf);
    queue_scheduler_test_data(&mut pair);
...
#[test]
fn control_packets_do_not_train_ack_ecf() -> TestResult {
    let _guard = subscribe();
    let mut pair = pair_with_scheduling_policy(MultipathSchedulingPolicy::AckEcf);
    let path_1 = open_available_second_path(&mut pair)?;
    pair.drive();
    ...
    for path_id in [PathId::ZERO, path_1] {
        let (rate, application_in_flight, _) = pair
            .conn(Client)
            .path_ack_ecf_metrics_for_test(path_id, now)?;
        assert_eq!(rate, None);
        assert_eq!(application_in_flight, 0);
    }
    Ok(())
}

#[test]
fn real_application_acks_train_each_ack_ecf_path() -> TestResult {
    let _guard = subscribe();
    let mut builder = ConnPair::builder()
        .enable_multipath()
        .disable_mtud_discovery()
    ...
    builder
        .client_transport_cfg
        .multipath_scheduling_policy(MultipathSchedulingPolicy::AckEcf);
    builder
        .server_transport_cfg
        .multipath_scheduling_policy(MultipathSchedulingPolicy::AckEcf);
    ...
    for path_id in [PathId::ZERO, path_1] {
        ...
        assert!(
            rate.is_some(),
            "path {path_id} should learn a delivery rate"
        );
        assert_eq!(application_in_flight, 0);
    }
    Ok(())
