...
impl TransportConfig {
    ...
    ///
    /// This does not change path validation, path status, congestion control, pacing, or
    /// path-specific control traffic. If the preferred data path cannot send, another eligible
    /// path is tried. Defaults to [`MultipathSchedulingPolicy::Default`], which preserves noq's
    /// original lowest-Path-ID-first behavior.
...
///
/// Path-specific control traffic keeps its original scheduling rules. Backup paths are only
/// eligible when no validated Available path can carry data.
