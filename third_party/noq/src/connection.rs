...
use proto::{
    ConnectionError, ConnectionHandle, ConnectionStats, DeclaredBackloggedEpochError, Dir,
    EndpointEvent, FourTuple, PathError, PathEvent, PathId, PathStats, PathStatus, Side,
    StreamEvent, StreamId, TransportError, TransportErrorCode, congestion::Controller,
    n0_nat_traversal,
};
...
impl Connection {
    ...
    /// Declares a fixed diagnostic epoch during which every 250 ms cohort is measured.
    ///
    /// This succeeds only when the transport configuration explicitly enabled the default-off
    /// sensor. The declaration records observations but does not alter scheduling or congestion
    /// control.
    pub fn begin_declared_backlogged_epoch(
        ...
        let mut state = self
            .0
            .lock_without_waking("begin_declared_backlogged_epoch");
        let now = state.runtime.now();
        state.inner.begin_declared_backlogged_epoch(now, duration)
    }
    ...
    /// Transmit `data` as an unreliable, unordered application datagram on one exact local path.
    ///
    /// The target path must already be validated and open. Unlike ordinary datagrams, this may
    /// explicitly select a [`PathStatus::Backup`] path. The datagram remains subject to the target
    /// path's congestion window, pacing and MTU, and is never silently moved to another path.
    ///
    /// Previously queued datagrams which are still unsent may be discarded to make space for this
    /// datagram, in order of oldest to newest.
    pub fn send_datagram_on_path(
        &self,
        path_id: PathId,
        data: Bytes,
    ) -> Result<(), SendDatagramOnPathError> {
        let conn = &mut *self.0.lock_and_wake("send_datagram_on_path");
        if let Some(ref x) = conn.error {
            return Err(SendDatagramError::ConnectionLost(x.clone()).into());
        }
        use proto::SendDatagramError::*;
        match conn.inner.datagrams().send_on_path(path_id, data, true) {
            Ok(()) => Ok(()),
            Err(proto::SendDatagramOnPathError::Datagram(error)) => {
                Err(SendDatagramOnPathError::Datagram(match error {
                    Blocked(..) => unreachable!(),
                    UnsupportedByPeer => SendDatagramError::UnsupportedByPeer,
                    Disabled => SendDatagramError::Disabled,
                    TooLarge => SendDatagramError::TooLarge,
            ...
            Err(proto::SendDatagramOnPathError::PathUnavailable(path_id)) => {
                Err(SendDatagramOnPathError::PathUnavailable(path_id))
            }
        }
    }

    /// Transmit `data` on one exact local path without sharing a QUIC packet with another
    /// application DATAGRAM frame.
    ///
    /// ACK and control frames may still share the packet. The boundary does not bypass the target
    /// path's congestion window, pacing, MTU, anti-amplification, or outgoing-buffer constraints.
    pub fn send_datagram_on_path_separate(
        &self,
        path_id: PathId,
        data: Bytes,
    ) -> Result<(), SendDatagramOnPathError> {
        let conn = &mut *self.0.lock_and_wake("send_datagram_on_path_separate");
        if let Some(ref x) = conn.error {
            return Err(SendDatagramError::ConnectionLost(x.clone()).into());
        }
        use proto::SendDatagramError::*;
        match conn
            .inner
            .datagrams()
            .send_on_path_separate(path_id, data, true)
        {
            Ok(()) => Ok(()),
            Err(proto::SendDatagramOnPathError::Datagram(error)) => {
                Err(SendDatagramOnPathError::Datagram(match error {
    ...
    /// Transmit `data` on one exact local path, waiting for outgoing buffer space if necessary.
    ///
    /// This combines the path guarantees of [`send_datagram_on_path()`] with the backpressure
    /// behavior of [`send_datagram_wait()`].
    ///
    /// [`send_datagram_on_path()`]: Connection::send_datagram_on_path
    /// [`send_datagram_wait()`]: Connection::send_datagram_wait
    pub fn send_datagram_on_path_wait(
        &self,
        path_id: PathId,
        data: Bytes,
    ) -> SendDatagramOnPath<'_> {
        SendDatagramOnPath {
            conn: &self.0,
            path_id,
            separate: false,
    ...
    /// Transmit `data` on one exact path with an application-DATAGRAM packet boundary, waiting for
    /// outgoing buffer space if necessary.
    pub fn send_datagram_on_path_separate_wait(
        &self,
        path_id: PathId,
        data: Bytes,
    ) -> SendDatagramOnPath<'_> {
...
pin_project! {
    /// Future produced by [`Connection::send_datagram_on_path_wait`]
    pub struct SendDatagramOnPath<'a> {
        conn: &'a ConnectionRef,
        path_id: PathId,
        separate: bool,
        data: Option<Bytes>,
...
impl Future for SendDatagramOnPath<'_> {
    type Output = Result<(), SendDatagramOnPathError>;

    fn poll(self: Pin<&mut Self>, ctx: &mut Context<'_>) -> Poll<Self::Output> {
        let mut this = self.project();
        let mut state = this.conn.lock_and_wake("SendDatagramOnPath::poll");
        if let Some(ref error) = state.error {
            return Poll::Ready(Err(SendDatagramError::ConnectionLost(error.clone()).into()));
        }

        use proto::SendDatagramError::*;
        let result = if *this.separate {
            state.inner.datagrams().send_on_path_separate(
                *this.path_id,
            ...
            state
                .inner
                .datagrams()
                .send_on_path(*this.path_id, this.data.take().unwrap(), false)
        };
        match result {
            Ok(()) => Poll::Ready(Ok(())),
            Err(proto::SendDatagramOnPathError::Datagram(error)) => {
                Poll::Ready(Err(SendDatagramOnPathError::Datagram(match error {
                    Blocked(data) => {
                        this.data.replace(data);
                        loop {
                            match this.notify.as_mut().poll(ctx) {
            ...
            Err(proto::SendDatagramOnPathError::PathUnavailable(path_id)) => {
                Poll::Ready(Err(SendDatagramOnPathError::PathUnavailable(path_id)))
            }
        }
    }
}
...
impl State {
    ...
    pub(crate) fn path_stats(&mut self, path_id: PathId) -> Option<PathStats> {
        let now = self.runtime.now();
        self.inner.advance_declared_backlogged_epoch(now);
        self.inner
            .path_stats(path_id)
...
/// Errors that can arise when sending a datagram on one exact local path.
#[derive(Debug, Error, Clone, Eq, PartialEq)]
