...
- `scripts/run_vpn_systemd_lab.sh` 使用真实 user systemd manager 和自动清理的临时 unit，覆盖正常停止、prepare 失败、READY 前失败、activate 失败和运行中异常退出；它动态验证 `prepare → READY → activate`、`deactivate → cleanup` 与 `NoNewPrivileges` 分界，不执行任何网络命令。
...
- `tests/vpn_systemd_lab.rs`、`tests/fixtures/flowweave-vpn-systemd-lab.service.in` 与 `scripts/run_vpn_systemd_lab.sh`：真实 user-systemd 生命周期门控；正式 client/server unit 的 root helper 边界、主进程权限和清理顺序另由静态合同测试锁定；
