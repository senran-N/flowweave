...
- VPN 的 mTLS 身份、控制协商、客户端/服务端数据句柄、真实 loopback DATAGRAM 地基，以及隔离网络空间内“非特权进程只附着既有 TUN”的内核验证；尚未接成产品命令和实际路由。
