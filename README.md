...
- `src/vpn_product_process.rs`、`src/bin/flowweave-vpn.rs`：非特权 server/client 产品生命周期、严格 READY 时序、systemd notify 和稳定 stdout 状态行。客户端在首次 READY 前及之后都只对明确定义的网络/远端可恢复错误做随机指数退避并重建完整 Endpoint；配置、协议、TUN/pump、本地资源不变量或 worker 故障快速失败，不执行 privileged 网络操作；
...
- `tests/vpn_tun_lab.rs` 与 `scripts/run_vpn_tun_lab.sh`：TUN 权限反例，以及隔离 client/server/internet 三 namespace 中真实 TUN + Endpoint/产品进程的 UDP、TCP、双栈无特权 ICMP、精确/超 MTU、连续代际、READY、IPv4 NAT、IPv6 无 NAT、身份恢复、外层链路恢复、服务端重启、离线停止与 `SIGKILL` 清理门控；
