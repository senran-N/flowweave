...
- VPN 的 mTLS 身份、控制协商、客户端/服务端数据句柄、真实 NoQ DATAGRAM、包设备桥接、单客户端 Endpoint 生命周期，以及隔离双网络空间中的真实 TUN 端到端纵切；它已把 DNS、严格名称校验、显式源 IP 路径、真实 TLS 1.3 mTLS、`FWC1`、IPv4/IPv6 UDP、TCP、ICMP、精确 MTU、连续代际、外层失联收敛和 `SIGKILL` 后 TUN 重附着接通，并新增可回滚的最小 root 网络 helper；尚未提供产品 VPN 命令、安装级 systemd 编排或真实宿主机默认路由/NAT/DNS。
...
- `scripts/run_vpn_tun_lab.sh` 只在一次性 user+mount+network namespace 中运行：先验证 root、未封闭提权、接口未启用、MTU 不一致和接口不存在都会失败，再创建彼此隔离的 client/server 网络空间、veth 外层链路和两端真实 TUN，以无 capability、`NoNewPrivileges` 的 UID 1000 运行真实 Endpoint；门控还会切断外层 veth、强杀两侧数据进程并验证 TUN 可重附着，主网络空间不创建接口或路由。
- `scripts/run_vpn_network_lab.sh` 只在一次性 user+mount+network namespace 中运行 `flowweave-vpn-net`：验证 root-owned/group-readable 配置、版本状态、并发锁、幂等 prepare/cleanup、中途回滚、崩溃恢复、同名/alias 归属保护和活动数据 fd 拒绝清理；主网络空间不运行 helper。
...
- `src/vpn_network.rs`、`src/bin/flowweave-vpn-net.rs`：从同一 product config/identity 真值派生最小双栈 TUN 计划，以 root-only 版本状态、计划指纹、随机 alias、独占锁和原子 journal 完成幂等 prepare/cleanup、崩溃恢复及归属保护；当前只添加点对点 host route；
...
- `tests/vpn_tun_lab.rs`：只由专用脚本运行的 TUN 权限反例，以及隔离 client/server 双 namespace 中真实 TUN + Endpoint 的 UDP、TCP、双栈无特权 ICMP、精确/超 MTU、连续代际、外层失联与 `SIGKILL` 清理门控；
...
实验室结果不等于生产 SLA。仓库已有默认 60 秒的单机真实 TLS/MPQUIC soak、可配置 JSONL 阈值检查、共享令牌无重启轮换，以及带限速、应用字节预算和周期检查点的公网 workload/echo 部署入口；现已完成同一物理出口下“两张接口 + 两条源路由 + 两个 NAT”的 30 分钟真实公网双路径 soak。VPN 已完成逐客户端身份、活动代际、在线撤销、按身份分片的数据热路径、外层 `FWI1` 准入、真实重组、原子全局账本和双向 ACL；客户端会精确比对 `FWC1 ACCEPT` 中的四个隧道地址，并让协商后的最大 IP 包和 DATAGRAM 长度实际约束双方数据面。单客户端 Endpoint 纵切现已从严格配置直接绑定服务端 UDP，客户端重新解析 DNS、严格校验证书名称、按 primary/additional 源 IP 验证路径，并在真实 TLS 1.3 mTLS/MPQUIC 上组合 `FWC1`、双方数据句柄、DATAGRAM 和包桥接。隔离内核门控进一步以无 capability、启用 `NoNewPrivileges` 的 UID 1000 串通真实 TUN 上的双栈 UDP、256 KiB TCP 半关闭、双栈 ICMP、精确/超 MTU、连续代际、外层失联和 `SIGKILL` 清理。两端 TUN 现由独立 root helper 从同一严格配置派生并事务化准备；专用门控还验证并发锁、配置/状态漂移、模拟崩溃恢复、活动 fd 拒绝 cleanup、同名外部接口保护和路由冲突完整回滚。上述实验都没有修改主网络空间。产品 `vpn-server` / `vpn-client` 命令、安装级 systemd 网络编排、真实宿主机选择性/默认路由、服务端 forwarding/NAT、DNS、内部长期重连和多路径恢复仍未完成。两个独立运营商出口只保留为运营商级故障隔离声明边界；多小时/多天证据、跨版本升级、外部指标存储与告警投递仍待完成。C 组编码器目前也是实验入口，不是通用实时媒体协议。
