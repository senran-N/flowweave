...
- VPN 的 mTLS 身份、控制协商、客户端/服务端数据句柄、真实 NoQ DATAGRAM、包设备桥接、单客户端 Endpoint 生命周期，以及隔离双网络空间中的真实 TUN 端到端纵切；它已把 DNS、严格名称校验、显式源 IP 路径、真实 TLS 1.3 mTLS、`FWC1`、协商包长、IPv4/IPv6 大 UDP 包、拒绝后继续接入和连续代际释放接通，但尚未提供产品命令、root oneshot 或真实宿主机路由/NAT/DNS。
...
- `tests/vpn_tun_lab.rs`：只由专用脚本运行的 TUN 权限反例，以及隔离 client/server 双 namespace 中真实 TUN + Endpoint 的 IPv4/IPv6 UDP 与两代际组合门控；
...
实验室结果不等于生产 SLA。仓库已有默认 60 秒的单机真实 TLS/MPQUIC soak、可配置 JSONL 阈值检查、共享令牌无重启轮换，以及带限速、应用字节预算和周期检查点的公网 workload/echo 部署入口；现已完成同一物理出口下“两张接口 + 两条源路由 + 两个 NAT”的 30 分钟真实公网双路径 soak。VPN 已完成逐客户端身份、活动代际、在线撤销、按身份分片的数据热路径、外层 `FWI1` 准入、真实重组、原子全局账本和双向 ACL；客户端会精确比对 `FWC1 ACCEPT` 中的四个隧道地址，并让协商后的最大 IP 包和 DATAGRAM 长度实际约束双方数据面。单客户端 Endpoint 纵切现已从严格配置直接绑定服务端 UDP，客户端重新解析 DNS、严格校验证书名称、按 primary/additional 源 IP 验证路径，并在真实 TLS 1.3 mTLS/MPQUIC 上组合 `FWC1`、双方数据句柄、DATAGRAM 和包桥接。普通 loopback 测试证明禁用身份拒绝后服务端可继续接入、活动期第二连接会被拒绝、同一双方包设备可连续复用两个代际；不可用源地址、错误证书名称和 UDP 黑洞会稳定失败且错误脱敏。进一步的隔离内核门控在同一个临时 user+mount namespace 下创建 server/client 两个独立 network namespace、veth 外层链路和两端真实 TUN，数据进程均为无 capability、启用 `NoNewPrivileges` 的 UID 1000；IPv4/IPv6 双向 UDP 有效载荷 `1300～1400` 字节全部一致，实际跨过 TUN、`FWI1` 分片、QUIC DATAGRAM 和对端 TUN，并在不重建 TUN 的情况下连续完成两个会话代际，停止后重新附着也成功。上述实验没有修改主网络空间。产品 `vpn-server` / `vpn-client` 命令、root oneshot 网络准备、真实宿主机路由/NAT/DNS、内部长期重连、TCP/ICMP 与故障矩阵仍未完成。两个独立运营商出口只保留为运营商级故障隔离声明边界；多小时/多天证据、跨版本升级、外部指标存储与告警投递仍待完成。C 组编码器目前也是实验入口，不是通用实时媒体协议。
